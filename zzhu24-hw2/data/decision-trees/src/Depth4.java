...
public static void main(String[] args)throws Exception
	{
        ...
        double sum = 0;
         ...
         System.out.println("Evaluation from the ID3 with maximun depth of 4\n");
	    for(int i = 0; i < 5; i++)
        {
            Id3 training = new Id3();
            training.setMaxDepth(4);

	      	Instances train = data.trainCV(5,i);
            Instances test  = data.testCV(5,i);

	      	boolean reTest = false;
			if(cost4(train) >= 0.001){
				reTest = true;
			}
			else{
				reTest = false;
	  ...
	  System.out.println("Average the ID3 with maximun depth of 4:" + average + "%\n\n");
	}
	private static double cost4(Instances data){
		double evaluation = 0;
		double result = 0;
